...
game.state.add('play', {
    preload: function() {
        ...
        this.game.load.image('CritImage', 'assets/496_RPG_icons/S_Dagger04.png');
        this.game.load.image('ClickDmgMultiplier', 'assets/496_RPG_icons/S_Sword10.png');
        this.game.load.image('DPSMultiplier', 'assets/496_RPG_icons/S_Sword16.png');
        this.game.load.image('CritMultiplier', 'assets/496_RPG_icons/S_Sword01.png');
    ...
    create: function() {
        ...
        var clickText = this.add.text(580, 8, 'Click Damage : ' + this.player.clickDmg , {
            font: '20px Arial Black',
            fill: '#fff',
            strokeThickness: 4
        });
        var DPSText= this.add.text(580, 30, 'DPS : ' + this.player.dps  + "/s", {
            font: '20px Arial Black',
            fill: '#fff',
            strokeThickness: 4
        });

        var critText= this.add.text(580, 52, 'Crit Chance : ' + this.player.critChance  + "%", {
            font: '20px Arial Black',
            fill: '#fff',
            strokeThickness: 4
        });

        var critMultText= this.add.text(580, 74, 'Crit Multiplier : ' + this.player.critMultiplier  + "x", {
            font: '20px Arial Black',
            fill: '#fff',
            strokeThickness: 4
        });
        ...
        var upgradeButtonsData = [
            {icon: 'dagger', name: 'Attack', level: 0, cost: 5, purchaseHandler: function(button, player) {
                player.baseClickDmg += 1;
                player.clickDmg = player.ClickDmgMultiplier * player.baseClickDmg;
                clickText.setText('Click Damage : ' + this.player.clickDmg);
            }},
            {icon: 'swordIcon1', name: 'Auto-Attack', level: 0, cost: 50, purchaseHandler: function(button, player) {
                player.baseDPS += 5;
                player.dps = player.DPSMultiplier * player.baseDPS;
                DPSText.setText('DPS : ' + this.player.dps+ "/s");
            }},
            {icon: 'CritImage', name: 'Critical Chance', level: 0, cost: 100, purchaseHandler: function(button, player) {
                if(player.critChance<101){player.critChance += 1;}
                critText.setText('Crit Chance : ' + this.player.critChance  + "%");
            }},
            {icon: 'gold_coin', name: 'Gold Multiplier', level: 0, cost: 1000, purchaseHandler: function(button, player) {
                player.goldMultiplier += 1;
            }},
            {icon: 'ClickDmgMultiplier', name: 'Click Multiplier', level: 0, cost: 1000, purchaseHandler: function(button, player) {
                player.ClickDmgMultiplier += 1;
                player.clickDmg = player.ClickDmgMultiplier * player.baseClickDmg;
                clickText.setText('Click Damage : ' + this.player.clickDmg);
            }},
            {icon: 'DPSMultiplier', name: 'DPS Multiplier', level: 0, cost: 1000, purchaseHandler: function(button, player) {
                player.DPSMultiplier += 1;
                player.dps = player.DPSMultiplier * player.baseDPS;
                DPSText.setText('DPS : ' + this.player.dps + "/s");
            }},
            {icon: 'CritMultiplier', name: 'crit Multiplier', level: 0, cost: 1000, purchaseHandler: function(button, player) {
                player.critMultiplier += 1;
                critMultText.setText('Crit Multiplier : ' + this.player.critMultiplier  + "x");
            }},
        ];
    ...
    onKilledMonster: function(monster) {
        ...
        coin.goldValue = Math.round(this.level * 1.33 * this.player.goldMultiplier);
    ...
    onClickMonster: function(monster, pointer) {
        ...
        if(percent>this.player.critChance){
            dmg = this.currentMonster.damage(this.player.clickDmg);
            crit = false;
        }
        else{
            dmg = this.currentMonster.damage(this.player.clickDmg * this.player.critMultiplier);
            crit = true;
        }
        ...
        if (dmgText) {
            if(crit){
                dmgText.text = this.player.clickDmg*this.player.critMultiplier;
            }
            else{
